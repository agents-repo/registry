...
- valid metadata with all required fields per [specs/metadata-schema.md](../specs/metadata-schema.md), including:
  - Package metadata: `status` (one of `active`, `deprecated`, `archived`, `yanked`), `category` (non-empty string), and `estimateOverallCost` (object with required `band` and optional `estimatedCost`)
  - Agent and flow metadata: `status`, `category`, and `estimateCost` (object with required `band` and `estimatedCost`)
