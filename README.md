...
npm run agents:ci         # full reinstall before changing locks or extracts
```
...
except the one-time README backfill in `specs/versioning-rules.md`.
...
`detail.json` is generated by `package-build` and `package-index-rebuild`.
Contributors MUST NOT author it. See `specs/package-detail-schema.md`.
...
- `packages/<namespace>/<package-id>/detail.json` holds latest-snapshot
    package detail for webapp pages. See `specs/package-detail-schema.md`.
- See `specs/index-schema.md` for the index schema.
